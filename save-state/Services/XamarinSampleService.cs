...
using save_state.Models;
using save_state.Consts;

namespace save_state.Services
{
    class XamarinSampleService
    {
        ...
        public async Task<(bool result, ApiResponseModels.Response_API_001 returnBody, string message)> Send_API_001(string email, int userCategory)
        {
            ...
            var response = await new Helpers.ApiSend().Send(ConstApiEndpoints.Param_API_001.url,
                ConstApiEndpoints.Param_API_001.method, null, JsonConvert.SerializeObject(request));
        ...
        public async Task<(bool result, ApiResponseModels.Response_API_999 returnBody, string message)> Send_API_999(string token, CommonEnums.MasterTypes type)
        {
            ...
            var response = await new Helpers.ApiSend().Send(ConstApiEndpoints.Param_API_999.url, ConstApiEndpoints.Param_API_999.method,
